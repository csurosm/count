 ...
 */
public class FrozenColumnsTable<TMODEL extends TableModel> extends JScrollPane implements ScrollPaneConstants, TableModelListener
{
    /**
     * Instantiation. 
     * 
     ...
     */
    public FrozenColumnsTable(TMODEL model, int num_header_columns)
    {
        super();
    ...
    private void initComponents()
    {
        ...
        data_table.addMouseListener(new MouseListener()
        {
     ...
     * In default implementation, this does nothing. 
     * 
    ...
    protected void selectByReference(int family_idx, int column_idx)
    {
        ...
        {
            if (column_class != Integer.class)
                cell_value = new Double(((Number)cell_value).doubleValue());
        }
        ...
        String chosen_option = (String)JOptionPane.showInputDialog(this, description, title, JOptionPane.QUESTION_MESSAGE, new RowSelectionIcon(), choices, choices[default_choice_idx]);
            if (column_class == String.class)
            {
     ...
     * @param command type of selection: <q>eq</q> for equality, <q>le</q> for less-than-or-equal, <q>ge</q> for more-than-or-equal, etc.
    ...
    public int selectSimilarFamilies(int family_idx, int col, String command)
    {
        ...
        Comparable ref = (Comparable)model.getValueAt(family_idx, col);
    ...
    private class HeaderScrollSynchronizer implements ChangeListener
    {
         ...
         * @param scroll_pane where the whole table is displayed
         */
     ...
     * after a row sorting events: the end result after two <q>corrections</q>
