 ...
 * can make a file chooser pop up. Does not crash, but buggy and does not work well on MacOS.
 *
 * Replaced by {@link count.gui.kit.ShmancyFileDialog}
 *
 * @deprecated 
 */
...
//	private static final int FILENAME_FIELD_LENGTH=40;
	...
	{
		String macosprop="com.apple.macos.use-file-dialog-packages";
		//System.out.println("#**FFD.static() "+macosprop+"="+System.getProperty(macosprop));
		// old AWT property for compatibility on MacOS - dunno if still needed
		System.setProperty("com.apple.macos.use-file-dialog-packages", "true");
	}
	
	private static final int TIME_POPUP_PROGRESS = 25; // 25 for development // popup in 0.5 sec

//	private static Dialog newDialog(Frame frame, String title)
//	{
//		Dialog newDialog = new Dialog(frame, title);
//		
//		return newDialog;
//	}
...
//		setFile(getFile()); // synchronize with super
	...
	{
		/*
		 *  initialize via a dialog: using Frame bugs on MacOS with 
		 *  
		 *  [NSSavePanel runModal] caught non-fatal NSGenericException '-[NSApplication runModalForWindow:] may not be invoked inside of transaction begin/commit pair, or inside of transaction commit (usually this means it was invoked inside of a view's -drawRect: method.)'
		 */
		super(new Dialog(frame, title, true),title, mode); // title still not showing up
		Dialog owner = (Dialog) getOwner();
		owner.pack();
		this.title = title;
