...
//    private FancyFileDialog jar_dialog;
    ...
    private FancyFileDialog table_dialog;
    private FancyFileDialog tree_dialog;
    private FancyFileDialog rates_dialog;
    
    private FancyFileDialog load_bundle_dialog;
    
    private FancyFileDialog output_dialog;
    private FancyFileDialog output_tree_dialog;
    private FancyFileDialog save_bundle_dialog;
...
//        input_panel.setLayout(new BoxLayout(input_panel, BoxLayout.PAGE_AXIS));
        ...
        tree_dialog = new FancyFileDialog(app, "Input phylogeny", "Tree file:");
        input_panel.add(boxed(tree_dialog.getEmbeddableChooser()));
        table_dialog = new FancyFileDialog(app, "Input data table", "Table file:");
        input_panel.add(boxed(table_dialog.getEmbeddableChooser()));
        if (want_rates)
        {
	        rates_dialog = new FancyFileDialog(app, "Input rates model", "Rates file:");
        ...
        if (save_tree)
        {
	        load_bundle_dialog = new FancyFileDialog(app, "Load session", "Session file (XML):");
...
//    	output_panel.setLayout(new BoxLayout(output_panel, BoxLayout.PAGE_AXIS));
    	output_dialog = new FancyFileDialog(app, "Save output", FancyFileDialog.SAVE, "Output file:");
    	output_panel.add(boxed(output_dialog.getEmbeddableChooser()));
    	if (save_tree)
    	{
    		output_tree_dialog = new FancyFileDialog(app, "Save phylogeny", FancyFileDialog.SAVE, "Tree output:");
    		output_panel.add(boxed(output_tree_dialog.getEmbeddableChooser()));
        	save_bundle_dialog = new FancyFileDialog(app, "Save all models", FancyFileDialog.SAVE, "Save as session:");
    ...
    public void setRatesData(DataFile<GammaInvariant> rates_data)
    {
