...
//        Count.THREAD_PARALLELISM = num_threads;
        ...
        if (want_command_line)
        {
        	...
        	{
        		cli_dialog.addOption(CommandLine.OPT_MODEL_LENGTH_CATEGORIES, Integer.toString(((Number)gamma_lengthT.getValue()).intValue()), "Discrete Gamma variation categories for edge length");
        	...
        	if (opt instanceof MLGamma || opt instanceof MLDistribution)
        	{
        		...
        		cli_dialog.addOption(CommandLine.OPT_MODEL_ROOT_PRIOR, 
        				CommandLine.encodeDistributionOption(root_prior), "Root prior istribution");
        	}
...
// BUNDLE
    ...
    public SimulationView showSimulation()
    {
    	...
    	long rnd_seed = ((Number)epsT.getValue()).longValue();
...
// BUNDLE
        ...
        if (want_simulation)
        {
        	...
        	epsT.setValue((new java.util.Random()).nextLong());
        }
...
//        threadsB.add(threadsT);
        ...
        String optimization_instructions = "<em>There are multiple numerical optimization methods implemented for "
