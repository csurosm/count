...
public class Likelihood implements GLDParameters
{
	...
	private double getLogParameter(double param, double complement)
	{
		double getLogParameter;
		if (param<0.5)
			getLogParameter = Math.log(param);
		else
			getLogParameter = Math.log1p(-complement);
		return getLogParameter;
	}
	
	public double getLogLossParameter(int node) 
	{ 
		return getLogParameter(getLossParameter(node), getLossParameterComplement(node));
		//return Math.log(getLossParameter(node));
	}
	public double getLogDuplicationParameter(int node) 
	{
		return getLogParameter(getDuplicationParameter(node), getDuplicationParameterComplement(node));
	}
	public double getLogExtinction(int node) 
	{ 
		return getLogParameter(getExtinction(node), getExtinctionComplement(node));
	}
	public double getLogLossComplement(int node)
	{ 
		return getLogParameter(getLossParameterComplement(node), getLossParameter(node));
	}
	public double getLogDuplicationComplement(int node) 
	{
		return getLogParameter(getDuplicationParameterComplement(node),getDuplicationParameter(node));
//		return Math.log(getDuplicationParameterComplement(node));
	}
	public double getLogExtinctionComplement(int node) 
	{ 
		return Math.log1p(-getExtinction(node));
	...
	public double getLinearGainParameter(int node) {
		...
		double log_gn = this.getLogGainParameter(node);
...
//				LL += κ* log1_q; //  Math.log(q_1);  // Math.log1p(-q); //  Math.log(1.0-q); // log(1-q)
				...
				double log_κ = getLogGainParameter(node);
...
//			System.out.println("#**L.sNP "+node+"\tepsi "+epsi+"\te1 "+epsi_1);
		...
		if (log_q == Double.NEGATIVE_INFINITY)
		{
			...
			double logr = rates.getLogGainParameter(node);
...
//			}
		 ...
		 */
		protected final void computeEdge(int node)
		{
			...
			if (tree.isRoot(node))
			{
				...
				if (getLogLossComplement(node)==Double.NEGATIVE_INFINITY) //   logp==0.0) 
					K = new double[1];
				else 
					K = new double[2];
				if (calc_width==0)
...
//			if (ASSERT_CALCULATIONS) assert (q!=1.0 || one_minus_q != 0.0);
			...
			//System.out.println("#**L.P.cE "+node+"\tlogq "+logq+"\tloq1_q "+log1_q+"\tgainpar "+g);
...
//				double log1_q = Math.log(one_minus_q);
				double[] terms =new double[C.length];
				if (log_κ  == Double.NEGATIVE_INFINITY)
				{
...
//						System.out.println("#**L.P.cE "+node+"\t"+tree.toString(node)+"\t"+rates.getDuplicationRate(node)+"\tq "+rates.getDuplicationParameter(node)); //+"\tq~ "+q);
					for (int s=0; s<K.length; s++)
					{
						...
						while (ell<C.length)
						{
							...
							double w = binom + ks_log1_q //(κ+s)*log1_q 
									+ t_logq;
...
//			System.out.println("#**L.P.cE fam "+family_idx+"\t"+node+"\tK "+Arrays.toString(K));
		 ...
		 */
		protected final void computeNode(int node)
		{
...
//				}
		...
		/**
		 * Likelihood recurrences with unstable numerical precision. 
		 * 
		 * @param node
		 * @return
		 */
...
//					System.out.println("#**L.P.cN "+node+"\tjunior"+c+" "+junior+"\tC "+Arrays.toString(C)+"\tsibe "+sib_extinct);
		...
		/**
		 * Placeholder for overriding with a high-precision variant.
		 * 
		 * @param junior
		 * @param C
		 * @param logit_eps
		 ...
		 */
		private double[] computeSibling(int junior, double[] C, double eps){
...
//				Arrays.fill(terms, Double.NEGATIVE_INFINITY); // log(0.0) for safety
		...
		public String toString()
		{
			...
			sb.append("Fam ").append(family_idx);
			sb.append("[").append(table.getFamilyName(family_idx)).append("]");
			for (int node=0; node<tree.getNumLeaves(); node++)
			{
				sb.append(", ");
				sb.append(tree.getIdent(node)).append(":").append(profile[node]);
				if (0<profile[node])
					nlin++;
				nmem += profile[node];
			}
			sb.append("; #lin ").append(nlin).append("\t#mem ").append(nmem);
			return sb.toString();
...
//			out.printf("%9g\t%9g\t%9g\t%9g",len, drate, lrate, grate);
	...
	protected void reportLikelihood(PrintStream out, int min_copies, boolean want_distribution)
	{
	    ...
	    } else
	    {
		    ...
		    double p0r = -L0;
		    out.printf("# Poisson ngains p0 %.4f", p0);
		    double p_not0=-Math.expm1(L0);
		    
		    double p0k=p0;
		    for (int k=1; k<8; k++) {
		    	p0k *= p0r/k; 
		    	out.printf("\tp%d %.4g/%.2g",  k, p0k,p0k/p_not0);
		    }
		    out.printf("\tr %.2f\n", p0r);		
		    ...
		    if (0<min_copies) {
		    	...
		    	double Nunobs = N0+N1;
		    	double Ntot = nFam + Nunobs;
		    	out.printf("# Implied total families:\t%.1f\tNfam %d\tNunobs(N0+N1) %.1f=%.1f+%.1f\n", Ntot, nFam, Nunobs, N0, N1);
		    }
//		    printSingletons(out);
	...
	protected void mainmain(CommandLine cli) throws Exception
	{
        ...
        boolean want_distribution =  cli.getOptionBoolean(CommandLine.OPT_STATISTICS, false);
