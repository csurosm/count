 ...
 */
public class DirectEM extends ML implements GLDParameters
{
...
//	}
	...
	public double optimize(double eps, int maxiter)
	{
		...
		{
			System.out.println("#*DEM.o start "+LLstart);
		}
		...
		while(iter<maxiter)
		{
			...
			if (PRINT_OPTIMIZATION_MESSAGES)
			{
				System.out.println("#*DEM.o "+iter+"\tLL "+LLnow+"\tdiff "+diff+"\trdiff "+delta
						+"\tmaxxd "+max_xd
						);				
			}
			if (LLnow<LLprev) // dubious M step
			{
				System.out.println("#*DEM.o done/decrease ("+iter+")"+"\t LL "+LLprev+"\tdiff "+(LLstart-LLprev));
			...
			{
				System.out.println("#*DEM.o done/converged ("+iter+")"+"\t LL "+LLprev+"\tdiff "+(LLstart-LLprev)+"\tdrop "+diff+"\trdiff "+delta);
				iter++;
				break;
			}
			...
			if (iter==maxiter)
			{
				System.out.println("#*DEM.o done/iterations ("+iter+")"+"\t LL "+LLprev+"\tdiff "+(LLstart-LLprev)+"\tdrop "+diff+"\trdiff "+(-delta));
			}
		...
		if (!PRINT_OPTIMIZATION_MESSAGES) // DEBUG
		{
			...
			System.out.println("#**DEM.o/debugLL LLcorr "+E.LL+"/"+LLcorr2+"\tLunobs "+E.LLunobs+"/"+Lunobs2+"\tpunobs "+punobs+"/"+punobs2+"\tpobs "+pobs+"/"+pobs2
		...
		// DEBUG
		System.out.println("#*DEM.o timing "+iter
				+"\tavgE "+(nano*timeE/(iter+1.0))+"\ttotE "+(nano*timeE)
...
//				System.out.println("#**DEM.fPP n "+n+"\tm "+m+"\tw "+w[n][m]);
		...
		System.out.println("#**DEM.fitP gain "+pty_gain+"\tloss "+pty_loss+"\tdup "+pty_dup);
...
//						System.out.println("#**DEM.M "+v+"\tp "+p+"/"+p_1+"\tNu "+Nu+"\tSv "+Sv+"\tsubtract "+(Nu-Sv)+"\tdiff "+Nu_Sv+"\t// "+factory.rates.toString(v));
	...
	//						System.out.println("#**DEM.M node "+v+"\tsetting min kappa "+kappa1);
					...
					} else
					{
						...
						{
							System.out.println("#**DEM.M node "+v+"\tsmallkappa "+κ+"\tbrkt "+kappa1+","+kappa2);
						}
					}
...
//								+"\t// rates "+factory.rates.toString(v)); // DEBUG
					...
					{
						System.out.println("#**DEM.M node "+v+"\tsmallq1 "+q_1.apply(κ)
								+"\tNv-Sv "+Nv_Sv+"\tNv "+Nv
								+"\tkappa "+κ+"\tF "+F
								+"\t// tNv "+Arrays.toString(tNv)+"\tSv "+Arrays.toString(tSv)
...
//				D1.setSameCalculationWidthThresholds(factory);
			...
			if (PRINT_OPTIMIZATION_MESSAGES)
			{
				...
				double punobs2 = Math.exp(Lunobs2);
				System.out.println("#**DEM.E LLcorr "+LLcorr+"/"+LLcorr2+"\tLLun "+LLuncorr+"\tLunobs "+Lunobs+"/"+Lunobs2+"\tpunobs "+punobs+"/"+punobs2+"\tpobs "+pobs+"/"+pobs2+"\tF* "+Fcorr);
